...
def load_view(date_filter, business_filter, account_filter, carrier_filter):
    ...
    elif date_filter == "next7":
        query = query.gte("date", today.strftime("%Y-%m-%d"))\
                     .lte("date", next7.strftime("%Y-%m-%d"))
            ...
            if so_options:
                ...
                if selected_orders:
                    ...
                    if is_bulk:
                        ...
                        use_date = st.checkbox("Set pickup date", value=date_val is not None, key=f"use_date_{k}")
                        if use_date:
                            new_date = st.date_input("Date", value=date_val or today, key=f"date_{k}")
                        else:
                            new_date = None
...
with tab1:
    date_options = ["Today", "Tomorrow", "Next 7 days", "Last 7 days", "All"]
    selected_range = st.radio("Show", date_options, horizontal=True)
    date_filter_map = {"Today": "today", "Tomorrow": "tomorrow",
                       "Next 7 days": "next7", "Last 7 days": "last7", "All": "all"}
...
with tab2:
    st.caption("Orders with no pickup date assigned yet — add date and load ID when confirmed by carrier")
    ...
    with st.expander("➕ Add new order"):
        with st.form("new_shipment"):
            ...
            fc, fd = st.columns(2)
            new_so = fc.text_input("Sales order")
            new_po = fd.text_input("PO")

            fe, ff = st.columns(2)
            new_carrier_sel = fe.selectbox("Carrier", CARRIERS)
            new_consignee_sel = ff.selectbox("Consignee", CONSIGNEES)

            fg, fh = st.columns(2)
            new_freight_sel = fg.selectbox("Freight terms", FREIGHT_TERMS)
            new_ctn = fh.number_input("Cartons", min_value=0, step=1)

            fi, fj = st.columns(2)
            new_pallets = fi.number_input("Pallets", min_value=0, step=1)
            new_appt = fj.text_input("Appt time (MAD only)")

            fk, fl = st.columns(2)
            new_load = fk.text_input("Load # (if known)")
            has_date = fl.checkbox("Pickup date known")
            if has_date:
                new_date = st.date_input("Pickup date")
            else:
                new_date = None
            ...
            if submitted:
                if not new_so:
                    ...
                    db.table("outbound").insert({
                        ...
                        "account": new_account_sel if new_account_sel != "Other" else "",
                        "carrier": new_carrier_sel if new_carrier_sel != "Other" else "",
                        "date": new_date.strftime("%Y-%m-%d") if new_date else None,
                        "freight_terms": new_freight_sel if new_freight_sel != "Other" else "",
                        "appt_time": new_appt if new_business == "MAD" else "",
                        "consignee": new_consignee_sel if new_consignee_sel != "Other" else "",
                        "sales_order": new_so,
                        "po": new_po,
                        "ctn": int(new_ctn),
    ...
    if not pending_df.empty:
        st.markdown(f"**{len(pending_df)} orders pending pickup date**")

        pending_df["ready_label"] = pending_df.apply(
            lambda r: "✅ Ready" if r.get("ready_pu") else "⏳ Not ready", axis=1)

        display_cols = ["business", "account", "carrier", "consignee", "sales_order",
                        "po", "ctn", "pallet_total", "load_number", "ready_label", "notes"]
        display_cols = [c for c in display_cols if c in pending_df.columns]

        rename_map = {
            "business": "Business", "account": "Account", "carrier": "Carrier",
            "consignee": "Consignee", "sales_order": "Sales order", "po": "PO",
            "ctn": "Cartons", "pallet_total": "Pallets", "load_number": "Load #",
            "ready_label": "Status", "notes": "Notes"
        }

        display_pending = pending_df[display_cols].rename(columns=rename_map)

        def color_pending_row(row):
        ...
        st.dataframe(
            display_pending.style.apply(color_pending_row, axis=1),
            use_container_width=True,
            hide_index=True
        )

        st.divider()
        st.markdown("**Quick actions for pending orders**")
        st.caption("Select an order in the sidebar Edit section to assign a date or mark ready")
    else:
        st.markdown("""<div class="alert-green">
            <p class="alert-title">No pending orders</p>
            <p class="alert-sub">All orders have pickup dates assigned</p>
